...
static void espnow_data_prepare(example_espnow_send_param_t *send_param)
{
    ...
    if (send_param->len < sizeof(example_espnow_data_t)) {
        ESP_LOGE(TAG, "Send buffer too small");
        return;
    }
...
// Main ESP-NOW task (official example logic)
static void espnow_task(void *pvParameter)
{
    example_espnow_event_t evt;
        ...
        switch (evt.id) {
            ...
            case EXAMPLE_ESPNOW_RECV_CB:
            {
                ...
                if (ret == EXAMPLE_ESPNOW_DATA_BROADCAST) {
                    ...
                    // Check for peer restart by comparing sequence numbers
                    bool peer_restarted = false;
                    if (s_stats.packets_received > 0 && recv_seq < s_stats.last_seq_num) {
                        ESP_LOGW(TAG, "🔄 Peer restart detected: seq %d < last %d", recv_seq, s_stats.last_seq_num);
                        peer_restarted = true;
                    }
                    
                    // Store peer MAC and update connection state
                    ...
                    // Always handle broadcast for peer restart scenarios
                    // If peer exists, this might be a restart - refresh connection state
                    bool peer_exists = esp_now_is_peer_exist(recv_cb->mac_addr);
                    bool need_immediate_response = false;
                    
                    if (peer_exists) {
                        if (peer_restarted) {
                            ESP_LOGI(TAG, "🔄 Confirmed peer restart - refreshing connection state");
                            // CRITICAL: Reset state to 0 to restart discovery handshake
                            send_param->state = 0;
                            need_immediate_response = true;
                            ESP_LOGI(TAG, "🔄 Reset state to 0 for discovery handshake restart");
                        } else {
                            ESP_LOGI(TAG, "📡 Broadcast from existing peer");
                        }
                    }
                    ...
                    // Reset state to handle peer restart properly
                    if (send_param->state == 0 || peer_restarted) {
                        send_param->state = 1;
                    }
                    
                    // Send immediate response for peer restart to accelerate reconnection
                    if (need_immediate_response) {
                        ESP_LOGI(TAG, "🚀 Sending immediate state=1 broadcast to accelerate reconnection");
                        espnow_data_prepare(send_param);
                        if (esp_now_send(send_param->dest_mac, send_param->buffer, send_param->len) != ESP_OK) {
                            ESP_LOGW(TAG, "Failed to send immediate response");
                        }
                    }
                    
                    // Modified: M5StickC Plus always acts as RECEIVER ONLY
                    // Never become sender - always fail the magic comparison
                    if (recv_state == 1) {
                        if (false && send_param->unicast == false && send_param->magic >= recv_magic) {
                            ESP_LOGI(TAG, "🎯 M5StickC Plus configured as RECEIVER-ONLY (Magic: 0x%08lX vs 0x%08lX)", 
                            ...
                            ESP_LOGI(TAG, "📥 M5StickC Plus acting as RECEIVER-ONLY (Magic: 0x%08lX < 0x%08lX)", 
                                     send_param->magic, recv_magic);
                    ...
                    // Update connection tracking for unicast data
                    s_stats.last_recv_time = xTaskGetTickCount() * portTICK_PERIOD_MS;
                    s_stats.last_seq_num = recv_seq;
                    s_stats.is_connected = true;
    ...
    ESP_LOGI(TAG, "ESP-NOW task ending");
    espnow_deinit(send_param);
    s_espnow_running = false;
    vTaskDelete(NULL);
}

// Cleanup function (official example)
static void espnow_deinit(example_espnow_send_param_t *send_param)
{
    if (send_param) {
